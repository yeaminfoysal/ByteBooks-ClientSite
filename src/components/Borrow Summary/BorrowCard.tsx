...
const BorrowCard: React.FC<BookCardProps> = ({ book }) => {
    ...
    <Card className="group relative overflow-hidden hover:shadow-2xl border-0">
      <CardContent className="p-0">
        ...
        <div className="relative aspect-[1] overflow-hidden group-hover:scale-105 transition-all duration-200">
          <div className={`absolute inset-0 bg-gradient-to-br ${getBookColor(book?.book.title)} opacity-80 w-[85%] mx-auto rounded-lg`} />

          {/* Book Icon */}
          <div className="absolute top-28 right-12">
            <BookOpen className={`w-48 h-30 ${getIconColor(book?.book.title)} opacity-100`} />
          </div>
